    ...
    public class CompassViewDrawerSimpleWithDistance : CompassViewDrawer
    {
        ...
        public override void OnDrawItem(Canvas canvas, PoiViewItem item, float startX, float endY)
        {
            if (item.Visibility == HorizonLib.Domain.Enums.Visibility.Visible)
            {
                ...
                canvas.DrawText($"{item.Poi.Name} ({(item.Distance / 1000):F0}km)", 10, -startX - 10, textpaint);
            }
            ...
            else if (item.Visibility == HorizonLib.Domain.Enums.Visibility.PartialyVisible)
            {
                ...
                canvas.DrawText($"{item.Poi.Name} ({(item.Distance / 1000):F0}km)", 10, -startX - 10, textpaintPartialyVisible);
            }
        }
